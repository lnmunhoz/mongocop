...
import { addHost, addTemplate, isCredentialHost, isLegacyHost, loadConfig, maskConnectionString, removeHost, removeTemplate, renameHost, renameTemplate, } from "./lib/config.js";
import { CredentialStoreError, createCredentialRef, getCredentialStore, getDefaultCredentialStore, } from "./lib/credentials/index.js";
...
function printVersionIfRequested(args = process.argv.slice(2)) {
...
async function resolveHostConnectionString(host) {
    if (isCredentialHost(host)) {
        const store = getCredentialStore(host.credential.store);
        const value = await store.get(host.credential);
        if (!value) {
            throw new CredentialStoreError("backend_error", `Saved credential not found in ${describeStore(host.credential.store)}.`);
        }
        return value;
    }
    if (typeof host.connectionString === "string") {
    ...
    throw new CredentialStoreError("backend_error", `Connection "${host.name}" has no stored credential.`);
}
async function migrateLegacyHosts(hosts) {
    const legacyHosts = hosts.filter(isLegacyHost);
    if (legacyHosts.length === 0)
        return;
    p.log.warn(`Found ${legacyHosts.length} legacy connection(s) stored in plaintext config.`);
    ...
    for (const host of legacyHosts) {
        ...
        try {
            ...
            p.log.error(`Could not migrate "${host.name}": ${toErrorMessage(err, "Unknown keychain error.")}`);
        }
    }
    if (migratedCount > 0) {
        p.log.success(`Migrated ${migratedCount}/${legacyHosts.length} connection(s) to macOS Keychain.`);
    }
    if (migratedCount < legacyHosts.length) {
        p.log.warn(`${legacyHosts.length - migratedCount} connection(s) remain in legacy plaintext format.`);
    }
}
async function manageConnections(hosts) {
    ...
    if (action === "rename") {
        ...
        if (p.isCancel(newName))
            return;
        await renameHost(selectedHost.name, newName);
        p.log.success(`Renamed "${selectedHost.name}" → "${newName}"`);
    }
    if (action === "delete") {
        ...
        if (isCredentialHost(selectedHost)) {
            ...
            try {
                ...
                p.log.warn(`Removed config entry but could not delete keychain item: ${toErrorMessage(err, "Unknown keychain error.")}`);
            }
        }
...
async function pickConnectionString(message = "Select a host", excludeConnectionString) {
    ...
    while (true) {
        ...
        if (selectableHosts.length > 0) {
            const options = selectableHosts.map((host) => ({
                value: host.name,
                label: host.name,
                hint: hostHint(host),
            }));
            ...
            if (choice !== "__new__") {
                ...
                try {
                    ...
                    p.log.error(`Could not load "${selectedHost.name}": ${toErrorMessage(err, "Unknown credential-store error.")}`);
                    continue;
                }
            }
        }
        ...
        if (shouldSave) {
            ...
            try {
                await defaultStore.set(credentialRef, connectionString);
                try {
                    await addHost({
                        name: name,
                        kind: "credential",
                ...
                p.log.success(`Saved as "${name}" in ${describeStore(defaultStore.name)}.`);
                if (!excludeConnectionString ||
                ...
                p.log.error(`Could not save "${name}": ${toErrorMessage(err, "Unknown credential-store error.")}`);
            }
        }
        if (excludeConnectionString &&
            connectionString === excludeConnectionString) {
...
function templateHint(template) {
    const scope = template.collections && template.collections.length > 0
...
async function manageTemplates(templates) {
    ...
    if (action === "rename") {
        ...
        if (p.isCancel(newName))
            return;
        await renameTemplate(selectedTemplate.name, newName);
        p.log.success(`Renamed "${selectedTemplate.name}" → "${newName}"`);
    }
    if (action === "delete") {
        const confirm = await p.confirm({
...
async function pickStartAction() {
    while (true) {
        const config = await loadConfig();
...
async function connectClient(connectionString, label) {
...
async function pickTargetDatabase(targetClient, sourceDbName, copyTarget) {
    const targetDatabases = await listUserDatabases(targetClient);
    if (targetDatabases.length > 0) {
        const selectableTargetDatabases = copyTarget === "same"
            ? targetDatabases.filter((db) => db.name !== sourceDbName)
            : targetDatabases;
        const targetDbOptions = selectableTargetDatabases.map((db) => ({
            value: db.name,
            label: db.name,
        ...
        if (targetDbChoice !== "__new__") {
            return targetDbChoice;
        }
    }
    const newDbName = await p.text({
        message: targetDatabases.length > 0 ? "New database name" : "Target database name",
        placeholder: `${sourceDbName}-copy`,
        validate: (name) => copyTarget === "same" && name === sourceDbName
...
async function runCopy({ sourceClient, targetClient, sourceDbName, targetDbName, selectedCollections, }) {
...
async function maybeSaveTemplate(copy) {
    ...
    p.log.success(`Saved template "${name}".`);
}
async function runInteractiveCopy() {
    ...
    try {
        ...
        if (copyTarget === "different") {
            const target = await pickConnectionString("Target host", source.connectionString);
            targetHostName = target.hostName;
            targetClient = await connectClient(target.connectionString, "target");
        }
        const targetDbName = await pickTargetDatabase(targetClient, sourceDbName, copyTarget);
        await runCopy({
            sourceClient,
            targetClient,
            sourceDbName,
...
async function runTemplateCopy(template) {
    const config = await loadConfig();
    const sourceHost = config.hosts.find((host) => host.name === template.sourceHost);
    const targetHost = config.hosts.find((host) => host.name === template.targetHost);
    if (!sourceHost) {
    ...
    try {
        ...
        p.log.error(`Could not load template hosts: ${toErrorMessage(err, "Unknown credential-store error.")}`);
        return false;
    }
    ...
    try {
        ...
        if (!databases.some((db) => db.name === template.sourceDatabase)) {
            p.log.error(`Template source database "${template.sourceDatabase}" was not found.`);
            return false;
        }
        const selectedCollections = template.collections ?? [];
        if (selectedCollections.length > 0) {
            const sourceCollections = (await sourceClient
                .db(template.sourceDatabase)
                .listCollections()
                .toArray())
                .filter((collection) => collection.type !== "view")
                .map((collection) => collection.name);
            const missingCollections = selectedCollections.filter((collection) => !sourceCollections.includes(collection));
            if (missingCollections.length > 0) {
                p.log.error(`Template collection(s) not found: ${missingCollections.join(", ")}`);
                return false;
            }
        }
